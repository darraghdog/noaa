...
boundaries = [80,60,50,50,20] # [100,80,70,70,40]
...
if make_train:
    ...
    def make_CV_boxes(cutoff1, file_name):
        rfcnCVfold2 = pd.read_csv("../coords/comp4_30000_det_test_seals_small_multi_fold2.txt",\
                            delimiter = " ", header=None, names=['img', 'proba', 'x0', 'y0', 'x1', 'y1'])
        rfcnCVfold1 = pd.read_csv("../coords/comp4_30000_det_test_seals_small_multi_fold1.txt",\
                            delimiter = " ", header=None, names=['img', 'proba', 'x0', 'y0', 'x1', 'y1'])
        ...
        return rfcnCV
    rfcnCV = make_CV_boxes(cutoff, '../coords/rfcnCV_small.pkl')
    rfcnCVlo06 = make_CV_boxes(0.6, '../coords/rfcnCV_smalllo06.pkl')
else:
    rfcnCV = pd.read_pickle('../coords/rfcnCV_small.pkl' )
    rfcnCVlo06 = pd.read_pickle('../coords/rfcnCV_smalllo06.pkl' )
...
if make_train_classes:
    ...
    def make_CV_class_boxes(cutoff, file_name):
        ...
        rfcnCV.to_pickle(file_name)
        return rfcnCV
    rfcnCVclass = make_CV_class_boxes(cutoff, '../coords/rfcnCV_smallclass.pkl')
    #rfcnCVlo06 = make_CV_boxes(0.6, '../coords/rfcnCVlo06.pkl')
else:
    rfcnCVclass = pd.read_pickle('../coords/rfcnCV_smallclass.pkl' )
    #rfcnCVlo06 = pd.read_pickle('../coords/rfcnCVlo06.pkl' )

# Load the Xtrain files
if make_test:
    def make_Tst_boxes(cutoff1, file_name):
        #rfcnTst = pd.read_csv("../coords/comp4_30000_det_test_seals_small_multi_test.txt", skiprows=1, \
        #                    delimiter = ",", header=None, names=['img', 'proba', 'x0', 'y0', 'x1', 'y1'])
        #rfcnTst = rfcnTst[rfcnTst['proba']>cutoff1]
        fname = "../coords/comp4_30000_det_test_seals_small_multi_test.txt"
        iter_csv = pd.read_csv(fname, iterator=True, chunksize=10000, \
                    delimiter = " ", header=None, names=['img', 'proba', 'x0', 'y0', 'x1', 'y1'])
        #rfcnTst = pd.concat([chunk[(chunk['proba'] > cutoff1)] for chunk in iter_csv])
        
        def processChunk(df):
            df = df[df['proba']>cutoff1]
            df = df[(df['x1']-df['x0'])<150]
            df = df[(df['y1']-df['y0'])<150]
            df['img'] = df['img'].str.replace('/home/ubuntu/noaa/darknet/seals/JPEGImagesTest/', '')
            return df
        rfcnTst = pd.concat([processChunk(chunk) for chunk in iter_csv])   
        ...
        return rfcnTst
    rfcnTst = make_Tst_boxes(cutoff, '../coords/rfcnTst_small.pkl')
    rfcnTstlo06 = make_Tst_boxes(0.6, '../coords/rfcnTst_smalllo06.pkl')
else:
    rfcnTst = pd.read_pickle('../coords/rfcnTst_small.pkl')
    rfcnTstlo06 = pd.read_pickle('../coords/rfcnTst_smalllo06.pkl')
