...
def ct_seal(row):
    row_id, row_block = row['img'].split('_')
    return sum((coords['id']==int(row_id)) & \
                (coords['block'] == row_block) & \
                (coords['block_width']>(int(row['x0'])-check_border)) & \
                (coords['block_width']<(int(row['x1'])+check_border)) & \
                (coords['block_height']>(int(row['y0'])-check_border)) & \
...
if make_train:
    ...
    def make_CV_boxes(cutoff1, file_name):
        ...
        gc.collect()
        rfcnCV['seal'] = rfcnCV.progress_apply(ct_seal, axis=1)
        ...
        return rfcnCV
    rfcnCV = make_CV_boxes(cutoff, '../coords/rfcnmultiCV.pkl')
else:
    rfcnCV = pd.read_pickle('../coords/rfcnmultiCV.pkl' )
...
if make_test:
    def make_Tst_boxes(cutoff1, file_name):
        ...
        return rfcnTst
    rfcnTst = make_Tst_boxes(cutoff, '../coords/rfcnmultiTst.pkl')
else:
    rfcnTst = pd.read_pickle('../coords/rfcnmultiTst.pkl')
