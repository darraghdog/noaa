...
#resn50tst = fread("vggTestPreds2604.csv")  
...
vggbigtrn = getBreaks(vggtrn, c(seq(.2,.8,.2), .9, .95, .99), "vgg")
vggbigtst = getBreaks(vggtst, c(seq(.2,.8,.2), .9, .95, .99), "vgg")

# Create training and test sets
names(meta_tst)[1] = names(meta_trn)[1]  = "img"
Xtrn = merge(meta_trn, vggbigtrn, all = T, by = "img")
Xtrn = Xtrn[!Xtrn$img %in% bad_train_ids]
Xtst = merge(meta_tst, vggbigtst, all = T, by = "img")
Xtrn[is.na(Xtrn)] <- 0
Xtst[is.na(Xtst)] <- 0
...
sub_out = sub_ct
for(var in target_cols) print(paste0(round(cor(sub_ct[[var]], subDt[[var]]), 3), " ... ", var))
par(mfrow=c(2,3))
for(var in target_cols) plot(sub_ct[[var]], subDt[[var]], main = var)
for(var in target_cols) sub_out[[var]] = sub_out[[var]]

write.csv(subDt, paste0("../sub/sub-xgb-", Sys.Date(), ".csv"), row.names = F)
