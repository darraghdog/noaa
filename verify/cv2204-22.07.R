...
rfcn <- fread("~/Dropbox/noaa/coords/comp4_30K_det_trainval_seals.txt")
...
y_pred[y$adult_females<10][V1>60]
...
# # Lets try a sub
write.csv(y_opt, paste0("~/Dropbox/noaa/sub/sub-RFCN-", Sys.Date(), "-0.82.csv"), row.names = F)
...
# Check
y_opt[rowSums(y_opt[,-1,with=F])>300]
y_opt[rowSums(y_opt[,-1,with=F])>280 & rowSums(y_opt[,-1,with=F])<300]
bad = c(14571, 15647, 8567)
