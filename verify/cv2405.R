...
rfcnsm = fread("~/Dropbox/noaa/coords/vggTestPredssmall2105.csv")
#rfcn = rfcn[V2>.5]
...
rfcnnorm = rfcn[predSeal>.2][order(img)]
rfcnsm = rfcnsm[predSeal>.2][order(img)]
rfcn = rbind(rfcnnorm, rfcnsm)
rfcnnorm[,ct:=.N, by=img]
rfcnsm[,ct:=.N, by=img]
rfcn[,ct:=.N, by=img]
y_prednorm = rfcnnorm[,.(.N), by=img]
y_predsm = rfcnsm[,.(.N), by=img]
