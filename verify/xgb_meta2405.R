...
vggbigtrn = getBreaks(vggtrn, c(seq(.2,.8,.2)), "vgg")
vggbigtst = getBreaks(vggtst, c(seq(.2,.8,.2)), "vgg")
vggtrnsm[, bigimg := unlist(lapply(strsplit(img, "_"), function(x) x[1]))]
vggtstsm[, bigimg := unlist(lapply(strsplit(img, "_"), function(x) x[1]))]
vggbigtrnsm = getBreaks(vggtrnsm, c(seq(.2,.8,.2)), "vggSmall")
vggbigtstsm = getBreaks(vggtstsm, c(seq(.2,.8,.2)), "vggSmall")
resn50trn[, bigimg := unlist(lapply(strsplit(img, "_"), function(x) x[1]))]
resn50tst[, bigimg := unlist(lapply(strsplit(img, "_"), function(x) x[1]))]
resn50bigtrn = getBreaks(resn50trn, c(seq(.2,.8,.2)), "resn50")
resn50bigtst = getBreaks(resn50tst, c(seq(.2,.8,.2)), "resn50")

ct_trn = merge(resn50bigtrn, vggbigtrn, all = T, by = "img") 
ct_tst = merge(resn50bigtst, vggbigtst, all = T, by = "img") 
ct_trn = merge(ct_trn, vggbigtrnsm, all = T, by = "img") 
ct_tst = merge(ct_tst, vggbigtstsm, all = T, by = "img") 
