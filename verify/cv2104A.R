...
y_opt$adult_males =  0.1 * (y_pred$V1)
y_opt[["adult_males"]][y_opt[["adult_males"]]<0]=0
y_opt$adult_females =  y_pred$V1*0.5
y_opt$juveniles =  y_pred$V1*0.28
y_opt$pups =  0.2 * (y_pred$V1)
hist(y_pred$V1, breaks = 300)


for (var in cols) print(paste(round(mean(y_opt[[var]]),1), "...", var))
...
# Lets try a sub
write.csv(y_opt, paste0("~/Dropbox/noaa/sub/sub-RFCN-weight-", Sys.Date(), ".csv"), row.names = F)
