...
# Get the packed blocks
resn50trn[predSeal>0.5,ct:=.N, by="img"]
resn50tst[predSeal>0.5,ct:=.N, by="img"]
resn50cttrn = unique(resn50trn[!is.na(ct),.(img, bigimg,ct)])
resn50cttst = unique(resn50tst[!is.na(ct),.(img, bigimg,ct)])

resn50cttrn[ct>20,ct20:=.N,by ="bigimg"]
resn50cttst[ct>20,ct20:=.N,by ="bigimg"]
resn50cttrn[ct>30,ct30:=.N,by ="bigimg"]
resn50cttst[ct>30,ct30:=.N,by ="bigimg"]
resn50cttrn = unique(resn50cttrn[!is.na(ct20),.(bigimg,ct20,ct30)])
resn50cttst = unique(resn50cttst[!is.na(ct20),.(bigimg,ct20,ct30)])
resn50cttrn[,img:=as.integer(bigimg)]
resn50cttst[,img:=as.integer(bigimg)]

ct_trn = merge(resn50bigtrn, vggbigtrn, all = T, by = "img") 
ct_tst = merge(resn50bigtst, vggbigtst, all = T, by = "img") 
ct_trn = merge(ct_trn, resn50cttrn, all = T, by.x = "img", by.y = "bigimg") 
ct_tst = merge(ct_tst, resn50cttst, all = T, by.x = "img", by.y = "bigimg")
...
write.csv(sub_out, paste0("../sub/sub-xgb-gblinear-", Sys.Date(), ".csv"), row.names = F)
