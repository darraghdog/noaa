...
for( var in target_cols){
  ...
  result_tmp = rep(NA, length(y))
  for(fold in folds){
    cvfit = cv.glmnet(Xtrn[fold,], y[fold])
    result_tmp[!fold] = pmax(0, round(predict(cvfit, Xtrn[!fold,], s = "lambda.min")))
  }
  print(paste0("Results for: ", var))
  result = c(result, rmse(y, result_tmp))
  print(result[length(result)])
...
# Model Cross validate Both
result = c()
for( var in target_cols){
  set.seed(100)
  y = Ytrn[[var]]
  result_tmp1 = result_tmp2 = rep(NA, length(y))
  for(fold in folds){
    
    cvfit = cv.glmnet(Xtrn[fold,], y[fold])
    result_tmp1[!fold] = pmax(0, round(predict(cvfit, Xtrn[!fold,], s = "lambda.min")))
    ...
    model <- xgb.train(data = train.xgb,
                       ...
                       objective = 'reg:linear',
                       eval_metric = 'rmse')
    result_tmp2[!fold] = pmax(0, round(predict(model, Xtrn[!fold,])))
  }
  
  result_tmp = (result_tmp1 + result_tmp2) / 2
  print(paste0("Results for: ", var))
  result = c(result, rmse(y, result_tmp))
  print(result[length(result)])
}
...
for( var in target_cols){
  ...
  cvfit = cv.glmnet(Xtrn, y)
  result1 = pmax(0, predict(cvfit, Xtst, s = "lambda.min"))
  ...
  model <- xgb.train(data = train.xgb,
                     ...
                     objective = 'reg:linear',
                     eval_metric = 'rmse')
  result2 = pmax(0, predict(model, Xtst))
  
  subDt[[var]] = round(.5*(result1 + result2))
  
  print(paste("Mean result 1", mean(result1) ))
  print(paste("Mean result 2", mean(result2) ))
  }
...
write.csv(sub_out, paste0("../sub/sub-xgb-glm-", Sys.Date(), ".csv"), row.names = F)
